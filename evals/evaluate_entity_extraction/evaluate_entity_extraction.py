...
def _collect_awards(funder: Funder) -> Tuple[List[str], List[str], List[str]]:
    """Flatten awards into independent id/scheme/title lists; per-award field-to-field pairings are not preserved."""
    ids, schemes, titles = [], [], []
...
_DEFAULT_GOLD_FILE = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "arxiv_test.jsonl"
)


def _load_gold_lookup(gold_file: str) -> dict:
    """Load a local gold JSONL into a {funding_statement: funders} lookup.

    `funding_statement` is used as the join key because the test file's
    `doi` is not unique (one paper / DOI can yield multiple funding
    statements). On duplicate statements, the first occurrence wins and
    ...
    with open(gold_file, encoding="utf-8") as f:
        for line in f:
            if not line.strip():
                continue
            row = json.loads(line)
            stmt = row.get("funding_statement")
            if not stmt:
                continue
            if stmt in lookup:
                duplicates += 1
                continue
            lookup[stmt] = row.get("funders", [])
    if duplicates:
        logging.warning(
            "Gold file %s had %d duplicate funding_statement(s); using first occurrence",
            gold_file,
            duplicates,
        )
    return lookup
...
def main():
    ...
    parser.add_argument(
        "--predictions",
        required=True,
        help="Path to a predictions JSONL produced by inference_funding.py "
             "(or any file with `funding_statement` + `predicted_funders` per line). "
             "An HF dataset repo id is also accepted for backward compatibility.",
    )
    parser.add_argument(
        "--split",
        default="predictions",
    ...
    parser.add_argument(
        ...
        help=(
            "Local JSONL file holding ground truth funders, joined to predictions "
            "by `funding_statement`. Defaults to arxiv_test.jsonl next to this script. "
            "Pass empty string to disable and fall back to the HF gold dataset."
        ),
    )
    parser.add_argument(
        "--gold-dataset",
        default="adambuttrick/funding-extraction-distillation-data",
        help="HF dataset with ground truth (only used if --gold-file is empty or missing)",
    )
    parser.add_argument(
        "--gold-config",
        default="test",
    ...
    # Some inference outputs use `funders` instead of `predicted_funders`;
    # alias it so the rest of the script can read a single field name.
    for line in lines:
        if "predicted_funders" not in line and "funders" in line:
            line["predicted_funders"] = line["funders"]

    use_local_gold = bool(args.gold_file) and os.path.exists(args.gold_file)

    if use_local_gold:
        print(f"Loading ground truth from local file {args.gold_file} ...")
        gold_lookup = _load_gold_lookup(args.gold_file)
        print(f"Indexed {len(gold_lookup)} unique gold statements")

        matched = 0
        unmatched_indices = []
        for i, line in enumerate(lines):
            stmt = line.get("funding_statement")
            gt = gold_lookup.get(stmt) if stmt else None
            if gt is not None:
                line["ground_truth_funders"] = gt
                matched += 1
            else:
                line["ground_truth_funders"] = []
                unmatched_indices.append(i)

        print(f"Matched {matched}/{len(lines)} predictions to ground truth by funding_statement")
        if unmatched_indices:
            preview = ", ".join(str(i) for i in unmatched_indices[:5])
            extra = f" (+{len(unmatched_indices) - 5} more)" if len(unmatched_indices) > 5 else ""
            logging.warning(
                "%d predictions had no matching gold statement; scored as all-false-positive. "
                "First indices: %s%s",
        ...
        has_paired = "ground_truth_funders" in sample

        if not has_paired:
            print(
                f"Loading ground truth from HF dataset {args.gold_dataset} "
                f"({args.gold_config}/{args.gold_split})..."
            )
            ...
            if duplicates:
                logging.warning(
                    "Gold dataset %s had %d duplicate DOI(s); using first occurrence",
                    args.gold_dataset,
                    duplicates,
                )
