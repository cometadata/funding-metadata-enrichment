...
class FakeColBERT:
    ...
    def encode(self, texts, batch_size=32, is_query=False, show_progress_bar=False):
        # Seed per-text so identical strings yield identical embeddings regardless
        # of call order/batching — required for batch-vs-per-doc byte-identity tests.
        out = []
        for t in texts:
...
def test_run_gpu_pass_demuxes_per_doc_topk():
    ...
    assert len(d1.topk_idx) == 2
    assert len(d1.topk_idx[0]) == 2
