...
class RunConfig:
    ...
    max_retries: int
    temperature: float
    top_p: float  # reserved for parse-failure rescue path
    max_tokens: int
    no_resume: bool
    log_every: int
...
async def _run_async(cfg: RunConfig) -> int:
    ...
    # ParquetWriter cannot append in-place: when resuming we write new rows to
    # a sidecar file and concat at the end. Refusing to overwrite on --no-resume
