...
from orchestrate_extractions import (
    Manifest,
    ManifestRow,
    parse_done_line,
    pick_next_batch,
...
def test_list_input_files_with_sizes(monkeypatch):
    import orchestrate_extractions as mod
...
def test_seed_merges_new_files_preserves_existing():
    from orchestrate_extractions import seed_or_merge_manifest
...
def test_reconcile_marks_done_when_output_exists():
    from orchestrate_extractions import reconcile_against_outputs
...
def test_submit_job_retries_429(monkeypatch):
    ...
    job_id = mod.submit_a100_job(
        script_path="hf_jobs/funding_statement_extractor/extract_funding_job.py",
